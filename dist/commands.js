...
import { formatBytes } from "./channel/media.js";
import { isStreamLevel, STREAM_LEVELS } from "./mirror.js";
import { contactKeyFor, DEFAULT_SESSION, nameOk, normalizeName } from "./registry.js";
...
export class CommandHandler {
    ...
    // ------------------------------------------------------------ pending 选择
    setPending(contactId, pending) {
        this.pendings.set(contactId, pending);
    }
    /** 裸数字且存在 pending 时解析选择。返回是否消费了消息。 */
    async tryResolvePending(contactId, text) {
        ...
        try {
            switch (pending.kind) {
                case 'model': {
                    if (!option.model || !pending.sessionId)
                        break;
                ...
                case 'thinking': {
                    if (!option.effort || !pending.sessionId || !pending.current)
                        break;
                    const selected = await this.deps.api.selectModel(pending.sessionId, {
    ...
    async handle(contactId, text, ctxToken) {
    ...
    async cmdWorkspace(contactId, key, arg) {
        try {
    ...
    async cmdSessions(contactId, key) {
        try {
            const reg = await this.deps.store.get(key);
    ...
    async cmdLevel(contactId, key, arg) {
        try {
            const reg = await this.deps.store.get(key);
            ...
            if (!arg) {
                ...
                await this.reply(contactId, `📊 当前输出等级：${current ?? 'normal（全局默认）'}（${desc[current ?? 'normal']}）\n`
    ...
    async cmdAttach(contactId, key, arg) {
        if (!arg) {
    ...
    async cmdThinking(contactId, key) {
        try {
            const current = await this.deps.sessions.ensureCurrent(key, contactId);
            await this.deps.sessions.ensureAgentLive(current.sessionId);
            const models = await this.deps.api.sessionModels(current.sessionId);
    ...
    async cmdSend(contactId, key, arg, ctxToken) {
        if (!arg) {
    ...
    // ------------------------------------------------------------ 原生透传
    async tryNative(contactId, key, text, name, _arg) {
        try {
