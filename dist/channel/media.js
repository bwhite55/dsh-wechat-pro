...
async function cdnUpload(opts) {
    ...
    for (let attempt = 1; attempt <= UPLOAD_MAX_RETRIES; attempt++) {
        try {
            ...
            if (!downloadParam)
                throw new Error('CDN upload 响应缺少 x-encrypted-param 头');
            break;
        }
        catch (err) {
            lastError = err;
...
export async function uploadMediaToCdn(opts) {
    ...
    if (st.size > MEDIA_MAX_BYTES)
        throw new Error(`${label}: 文件超过 ${formatBytes(MEDIA_MAX_BYTES)} 上限: ${formatBytes(st.size)}`);
...
async function fetchCdnBytes(url, label, log) {
    ...
    if (len > MEDIA_MAX_BYTES)
        throw new Error(`${label}: 媒体超过 ${formatBytes(MEDIA_MAX_BYTES)} 上限（content-length=${len}）`);
    const buf = Buffer.from(await res.arrayBuffer());
    if (buf.length > MEDIA_MAX_BYTES)
        throw new Error(`${label}: 媒体超过 ${formatBytes(MEDIA_MAX_BYTES)} 上限（实际 ${buf.length}）`);
    return buf;
}
...
export async function downloadAndDecrypt(opts) {
    ...
    const log = opts.log ?? (() => { });
    if (!aesKeyBase64)
        throw new Error(`${label}: 缺少 aes_key`);
...
export async function downloadMediaFromItem(opts) {
    ...
    const hasRef = (m) => Boolean(m && (typeof m.encrypt_query_param === 'string' || typeof m.full_url === 'string'));
    try {
        ...
        if (item?.type === MessageItemType.VOICE) {
            ...
            if (!hasRef(media) || typeof media.aes_key !== 'string')
                return null;
