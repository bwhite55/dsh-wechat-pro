...
import { log, mask } from './util.ts'

function text(value: string): ContentBlock[] {
  return [{ type: 'text', text: value }]
}

export function weixinSendTool(engine: EngineSurface) {
  return defineTool({
    name: 'weixin_send',
    ...
    output: {
      ...
      render: (_args, value: { ok: boolean; error?: string }) => text(value.ok ? '已发送到微信' : '发送失败：' + (value.error ?? '未知错误')),
    },
    async execute(args) {
      const target = engine.getLastSender()
      if (!target) return { ok: false, error: '微信通道还没有收到过用户消息，无法主动发送' }
      try {
...
export function weixinStatusTool(engine: EngineSurface) {
  return defineTool({
    name: 'weixin_status',
    ...
    output: {
      ...
      render: (_args, value: { state: string; loggedIn: boolean; error?: string }) =>
        text('微信状态：' + value.state + (value.error !== undefined ? '，错误：' + value.error : '')),
    },
    async execute() {
