...
export class IlinkError extends Error {
  ...
  constructor(
    message: string,
    opts: { kind?: 'api' | 'http' | 'transport'; ret?: number; errcode?: number; status?: number } = {},
  ) {
    super(message)
...
export interface IlinkClient {
  readonly config: { baseUrl: string; token: string | null; channelVersion: string; botAgent: string }
  ...
  getConfig(ilinkUserId: string, contextToken?: string): Promise<{ typingTicket: string }>
  sendTyping(ilinkUserId: string, typingTicket: string, status: number): Promise<void>
  sendMessageItems(options: SendItemsOptions): Promise<{ clientId: string }>
  sendMessage(options: SendTextOptions): Promise<{ clientId: string }>
  getUploadUrl(params: UploadUrlParams): Promise<{ uploadFullUrl: string; uploadParam: string }>
...
export function createIlinkClient(opts: IlinkClientOptions = {}): IlinkClient {
  ...
  function apiGet(endpoint: string, opts: { timeoutMs?: number; withToken?: boolean; label?: string } = {}): Promise<Record<string, unknown>> {
  ...
  function apiPost(
    ...
    opts: { timeoutMs?: number; withToken?: boolean; label?: string } = {},
    signal?: AbortSignal,
  ): Promise<Record<string, unknown>> {
    const url = new URL(endpoint, ensureTrailingSlash(cfg.baseUrl)).toString()
    return fetchWithTimeout(
      url,
      { method: 'POST', headers: headers(opts.withToken ?? true), body: JSON.stringify(body ?? {}) },
  ...
  return {
    ...
    async getBotQrcode(botType = DEFAULT_BOT_TYPE) {
      ...
      if (!qrcode) throw new Error('获取二维码失败：响应中缺少 qrcode')
      return { qrcode, qrcodeImgContent }
    },

    /** 轮询扫码状态（长轮询）。网络错误/客户端超时视为 wait。 */
    async pollQrcodeStatus(qrcode: string, verifyCode?: string) {
      ...
      try {
        ...
        if (isAbort(err) || err instanceof IlinkError) return { status: 'wait' }
    ...
    async getUpdates(buf = '', timeoutMs = LONG_POLL_TIMEOUT_MS, signal?: AbortSignal) {
      try {
        const resp = await apiPost(
        ...
        return {
          ...
          msgs: Array.isArray(resp.msgs) ? (resp.msgs as Array<Record<string, unknown>>) : [],
    ...
    async getConfig(ilinkUserId: string, contextToken?: string) {
      const resp = await apiPost(
        'ilink/bot/getconfig',
    ...
    async sendTyping(ilinkUserId: string, typingTicket: string, status: number) {
      await apiPost(
    ...
    async sendMessageItems({ to, itemList, contextToken, clientId }: SendItemsOptions) {
    ...
    async sendMessage({ to, text, contextToken, clientId }: SendTextOptions) {
    ...
    async getUploadUrl({ filekey, mediaType, toUserId, rawsize, rawfilemd5, filesize, aeskeyHex, noNeedThumb = true }: UploadUrlParams) {
      const resp = await apiPost(
...
function pick(resp: Record<string, unknown>, key: string): unknown {
  return resp[key]
}
function opt(resp: Record<string, unknown>, key: string): string | undefined {
  const v = resp[key]
 ...
 */
export function extractText(msg: InboundMessage | Record<string, unknown> | undefined): string {
  ...
  for (const item of items) {
    ...
    if (item.type === 3) {
      ...
      if (typeof voiceItem?.text === 'string' && voiceItem.text !== '') return voiceItem.text
    }
  }
  return ''
}
...
export function hasMedia(msg: InboundMessage | Record<string, unknown> | undefined): boolean {
  const items = Array.isArray(msg?.item_list) ? (msg.item_list as Array<Record<string, unknown>>) : []
