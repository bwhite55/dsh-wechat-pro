...
import { ApprovalForwarder } from './approval.ts'
import { buildEngineConfig, WechatEngine, type EngineConfig, type EngineSurface } from './channel/engine.ts'
import { downloadMediaFromItem } from './channel/media.ts'
import { extractText } from './channel/ilink.ts'
import { CommandHandler } from './commands.ts'
import { isStreamLevel, SessionMirror } from './mirror.ts'
import { ContactStore, contactKeyFor } from './registry.ts'
import { SessionManager } from './sessions.ts'
import { weixinSendTool, weixinStatusTool } from './tools.ts'
import { log, mask, safeKey } from './util.ts'
...
const WEIXIN_GUIDANCE =
  '本机已安装 dsh-wechat-pro 插件（微信 ClawBot 通道）：用户可通过微信与 agent 对话，'
...
function resolveEngineConfig(config: ResolvedConfig, dataDir: string): EngineConfig {
...
export function apply(ctx: Context, config?: Config): void {
  ...
  const resolve = (): ResolvedConfig => {
    ...
    return {
      ...
      streamLevel: isStreamLevel(value.streamLevel ?? '') ? value.streamLevel as 'minimal' | 'normal' | 'verbose' : 'minimal',
  ...
  const sync = (): void => {
    ...
    const engineSurface: EngineSurface = state.engine
  ...
  function ensureEngineState(ctx: Context, cfg: EngineConfig, dir: string, value: ResolvedConfig, store: ContactStore, hostCwd: string): EngineSurfaceState {
    ...
    if (engineSurfaceRef && engineSurfaceRef.configKey === key) return engineSurfaceRef
    if (engineSurfaceRef) {
    ...
    const engine = new WechatEngine(ctx, cfg, (msg) =>
      dispatchMessage(ctx, engineSurfaceRef!, msg, value, dir, store, manager),
    )
    const engineSurface: EngineSurface = engine
    const mirror = new SessionMirror(ctx, engineSurface, {
      ...
      levelOf: (contactId) => {
        ...
        return reg !== undefined && isStreamLevel(reg.streamLevel ?? '')
          ? reg.streamLevel as 'minimal' | 'normal' | 'verbose'
          : value.streamLevel
      },
    ...
    approval.start()
    const state: EngineSurfaceState = { engine, mirror, commands, approval, manager, configKey: key }
    engineSurfaceRef = state
    return state
  }
...
interface EngineSurfaceState {
  engine: WechatEngine
  mirror: SessionMirror
  commands: CommandHandler
  approval: ApprovalForwarder
...
async function dispatchMessage(
  ctx: Context,
  state: EngineSurfaceState,
  msg: InboundMessageSurface,
  ...
  // 1) 审批回复
  if (await state.approval.tryResolve(from, text)) return
  // 2) 数字选择（多步指令）
  if (await state.commands.tryResolvePending(from, text)) return
...
function makeRoute(method: string, pathname: string, handler: (req: IncomingMessage, res: ServerResponse) => Promise<void> | void): WebRoute {
  return {
    kind: 'exact',
    path: pathname,
    handler: (req, res) => {
...
/** 供 dispatcher 使用的消息最小形态。 */
type InboundMessageSurface = {
